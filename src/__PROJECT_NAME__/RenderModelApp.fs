        ...
        match currentModel with
            | MFileModel fileName -> 
                ...
                if System.IO.File.Exists file then  // check if good
                     return
                        file |> Aardvark.SceneGraph.IO.Loader.Assimp.load  |> Sg.adapter |> Sg.noEvents
                             |> Sg.transform (Trafo3d.FromOrthoNormalBasis(V3d.IOO, V3d.OOI, -V3d.OIO))
                else 
                    Log.warn "file not found"
                    return Sg.empty 
            | MSphereModel(center,radius) ->
                let sphere = Sg.sphere 5 (Mod.constant C4b.White) radius 
...
let app =
    {
        ...
        threads = threads
        initial = { 
                    currentModel = Some (BoxModel Box3d.Unit); 
